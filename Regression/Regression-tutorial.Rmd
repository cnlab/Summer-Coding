...
library(knitr)
knitr::include_graphics("/Users/stevenmesquiti/Desktop/Summer Coding Curriculum/Summer-Coding/Regression/falklab_logo_whitebrain (2).png")
```
...
```{r}
setwd("/Users/stevenmesquiti/Desktop/Summer Coding Curriculum/Summer-Coding/Regression") #change to your own WD. you can do that by modifying the file path or go session (on the upper bar) --> set working directory)
```
 ...
 table_model = function(model_data,reference = "Intercept") {
   model_data %>% 
     ...
     mutate(term = gsub("\\(Intercept\\)", !!reference, term),
            term = gsub("year", "Original Publication year", "Parameter", term)) %>%
     kable() %>% 
 ...
 table_model = function(model_data,reference = "Intercept") {
   model_data %>% 
     ...
     rename("SE" = std.error,
            ...
            "p" = p.value) %>%
     mutate(term = gsub("\\(Intercept\\)", !!reference, term),
            term = gsub("year", "Original Publication year", "Year", term)) %>%
     kable() %>% 
     kableExtra::kable_styling()
