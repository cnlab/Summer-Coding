...
Built-in datasets refer to the datasets already provided within R. For the first part, we will be using a dataset called the **air quality dataset**, which pertains to the daily air quality measurements in New York from May to September 1973. **This dataset consists of more than 100 observations on 6 variables** 
...
Once the data has been loaded into the workspace, we need to explore it to get an idea about its structure and what we have to work with. 
...
- `summary(airquality)`The summary method displays descriptive statistics for every variable in the dataset, depending upon the type of the variable. We can see at a glance the mean, median, max and the quartile values of the variables.
...
A histogram is quite similar to a bar chart except that it groups values into continuous ranges. A histogram represents the frequencies of values of a variable bucketed into ranges.We get a histogram of the Solar.R values.

```{r echo=T}
hist(airquality$Solar.R)
```

By giving an appropriate value for the color argument, we can obtain a colored histogram as well.
...
R comes equipped with sophisticated visualization libraries having great capabilities. In this section, we will use the built-in `mtcars` dataset to show the use of the `ggplot2` library. This dataset has been extracted from the 1974 Motor Trend US magazine.
...
`geom_point()` is used to create scatterplots and geom can have many variations like `geom_jitter()`, `geom_count()`, etc. Here, we use it to create a scattrerplo for weight and mpg of cars. Notice how we specify what variables we want on our X and Y axes.
...
For this part of the script script, and here forward, We use `pacman` to load in all of our
...
Now we we'll focus on making a variety of different plots using `ggplot2` to demonstrate its flexibility. You can read more about `ggplot2` [here](https://ggplot2.tidyverse.org/) and find its cheatsheet [here](https://www.maths.usyd.edu.au/u/UG/SM/STAT3022/r/current/Misc/data-visualization-2.1.pdf). Cheatsheets are great for quickly figuring out what you need to implement certain thinhs 
...
We can use a variety of functions within `ggplot2` to show how things change over time, one of which is line graphs. To do so, we can `tidy` our data (one observation per row using a package like `dplyr`. Now, I'll demonstrate how to do some using data from a project I worked on in Jamie Pennebaker's lab.

## Load in the Data

First, we load in the raw data from github. Second, we subset the data for a range of dates we are interested in (March 2019-March 2021). We then filter out word counts that are too noisy. Since LIWC is a bag of words program, we want to make sure our observations aren't too noisy or large. For example, in the sentence "I love you" pronouns take up about 66% of the words the sentence and doing any type of analyses with that would not be very informative. We can get more into LIWC later. 
...
`labs(x = "Month", y = 'Standardized score') +` adds our x and y axis lables
...
`y=43,label="Summer 2020 surge", size = 3) +` specifying what our label is and what the y plane value is and font size
                                
`annotate(geom="text",x=as.Date("2020-12-03"),` his specific date on the x plane
  

`y=43,label="Winter 2020 surge", size = 3)` specifying what our label is and what the y plane value is and font size
