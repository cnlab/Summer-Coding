...
can do that buy modifying the file path or go session (on the upper bar)
...
packages into your current `work place` by using the `library(PACKAGE)`
...
and `hmisc` both use summarize) that are masked. To get around this you
can use `PACKAGE::FUNCTION` to call package-specfic function.
...
still some merit to using that if loading in packages creates issues
like loading `tidyverse` and `brms` in a certain fashion.
...
For example, if your WD is already set you can use:
...
another folder you can do something like:
...
analyses. Today we are going to be looking at gender differences between
...
with **only** two levels (e.g., gender: Male and Female) and one DV that
is continuous (e.g., weight ranging from 0 to idk 250). If the IV has
more than two levels we need to use another type of test called ANOVA;
more on that later :).

Some other things: - Two means calculated from two samples Null
hypothesis: Means from two samples are similar • Alternative hypothesis:
Means from two samples are different • The bigger the observed
difference between sample means = More likely sample means differ
...
are: • The DV data are continuous (not ordinal or nominal). • The sample
data have been randomly sampled from a population. • There is
homogeneity of variance (i.e., the variability of the data in each group
is similar). • The distribution is approximately normal.

## Continuous

We can check this by looking at the structutre of our data using the
...
From inspecting our output, we can see that we have a significant effect
of gender on weight. That is, when conducting an independent t-test we
observed a large effect, sucha that that Men (M = 85.826, SD = 4.354)
possessed significantly greater [*t*(26.87) =-20.79, *p* \< .001, d =
-6.575] body weight compared to their female counterparts (M = 63.499,
SD = 2.028).
...
This tutorial uses data and reproduces a subset of analyses reported in
